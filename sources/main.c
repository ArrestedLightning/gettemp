  ...
  if (argc == 1) {//no arguments print standard text
    ...
    for (i = 1; i < argc; i++) {
      if (argv[i][0] == '-') {//skip argument if it don't start with a '-'
        if (debug) {
          printf("decoding arguments %c cnt: %i\n", argv[i][1], i);
        }
        switch (argv[i][1]) {
          case '?':
...
void PollOnce(char port, char sensor) {
  ...
  if (retries >= 25) {
    ...
    if (quietmode == 0) {
      printf("measured sensor %i temp %f\n",sensor,temp);
    }
    if (quietmode == 1) {
      printf("%f\n",temp);
    }
...
void ShowUsage(void) {
  ...
  printf("-px:		Poll a sensor once where x is the sensor address (0 is the first sensor)\n");
  ...
  printf("-l:		Poll all the sensors continuously\n");
  printf("\n");
  printf("\n");
}//showusage
