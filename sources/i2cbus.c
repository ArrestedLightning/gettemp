...
int result,arg;
  ...
  if (debug) printf("comport:%i \n",com);
  switch (com)
  {
    case 4:
     if (debug) printf("trying to open com 4 fd=");
     fd = open(COM_PORT4, O_RDWR | O_NOCTTY);
    break;

    case 3:
      if (debug) printf("trying to open com 3 fd=");
      fd = open(COM_PORT3, O_RDWR | O_NOCTTY);
    break;

    case 2:
      if (debug) printf("trying to open com 2 fd=");
      fd = open(COM_PORT2, O_RDWR | O_NOCTTY);
    break;

    case 1:
    default:
      if (debug) printf("trying to open com 1 fd=");
